% solve an FE model comprising TRI elements (v2)
...
Emat = [(1-nu)*c nu*c 0;nu*c (1-nu)*c 0;0 0 (1-2*nu)*c/2]; % E matrix

% read the mesh from an external file
[node,ele] = read_mesh_TRI6('LST_SUMMARY6_solution.inp');
nnod = length(node(:,1));
nele = length(ele(:,1));
...
% make a cell to hold all the N vectors, this is just a 1D cell vector of size equal
% to the number of ele in the mesh, each item in the cell vector is a
% symbolic 1x6 array of shape functions where N = N(r,s)
%N = cell(nele);
% make a cell to hold all the B arrays, this is just a 1D cell vector of size equal
...
for i=1:nele
    % define node locations for element i
...
plot_fe_results(node,ele,D,snode_avg,1000)

% print some results for comparison to Abaqus
%
% print the integration point strains for each ele
e1 = eip(:,:,1) % element 1
e2 = eip(:,:,2) % element 2
% stress at integration points... now there are 5 rows as stated above in
% the for loop
s1 = sip(:,:,1) % element 1
s2 = sip(:,:,2) % element 2
